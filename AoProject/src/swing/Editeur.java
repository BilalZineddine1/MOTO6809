...
public class Editeur extends JFrame {
	...
	private int i = 0, k = 0, ns = 0, nu = 0, rows, rowu, pos;
	...
	private void instr_st(char registre, String[] mots) {
		if (mots[1].charAt(0) == '$') {
			...
			if (mots[1].charAt(0) == ',') {
				String tmps;
				int tmpi = 0;
				if (taille == 4) {// pre(in/de)crementation

					System.out.println(mots[1].substring(2));
					if (mots[1].substring(2).equals("++")) {// on ne considere pas le cas: $20,X+ ou $20,-Y
						reg2 = mots[1].charAt(1);// cas ,X++
						tmps = getR(reg2);
						tmpi = hextoInt(tmps);
						tmpi++;
					} else if (mots[1].substring(1, 2).equals("--")) {// cas ,--X
					...
					tmps = intToHex(tmpi);
					setR(reg2, tmps);
				} else if (taille == 2) {// on obtient l'adresse depuis le reg cas ,X
					reg2 = mots[1].charAt(1);
				} else if (mots[1].length() == 3) {// post(in/de)crementation
				...
				System.out.println("le deplacement est de: " + hextoInt(deplacement) + " le registre est: "
						+ mots[1].charAt(taille - 1));
				adresse = getR(reg2);
				...
				if (mots[1].charAt(taille - 1) == ']') {
					if (mots[1].charAt(1) == ',') { // cas: [,X] ou [,-X] ou [,X++]...
						reg2 = mots[1].charAt(1);
						if (mots[1].length() == 6) {// pre(in/de)crementation
							String tmps = getR(reg2);
							int tmpi = hextoInt(tmps);
							if (mots[1].substring(3).equals("++"))// on ne considere pas le cas: $20,X+ ou $20,-Y
								tmpi++;
							if (mots[1].substring(2, 3).equals("--"))// cas ,--X
								tmpi--;
							tmps = intToHex(tmpi);
							setR(reg2, tmps);
						}
						adresse = getR(mots[1].charAt(1));// on obtient l'adresse depuis le reg cas ,X
						if (adresse.length() < 3)
							adresse = "00" + adresse;
						if (mots[1].length() == 3) {// post(in/de)crementation
							String tmps = getR(reg2);
							int tmpi = hextoInt(tmps);
							if (mots[1].charAt(3) == '+')// cas ,X+
								tmpi++;
							if (mots[1].charAt(1) == '-')// cas ,-X
								tmpi--;
							tmps = intToHex(tmpi);
							setR(reg2, tmps);
						}
						System.out.print("old adresse: " + adresse);
						adresse = (String) RAM.getData(adresse); // on recupere une nouvelle adresse depuis l'adresse
																	// originale
						RAM.setData(adresse, valreg);
					} else
					...
					if (mots[1].charAt(1) == '$' && mots[1].indexOf(',') > 0) { // cas [$20,X]
						...
						System.out.println(
								"le deplacement est de: " + hextoInt(deplacement) + " le registre est: " + reg2);
						adresse = getR(reg2);
						int tmpi = hextoInt(adresse);
						tmpi += hextoInt(deplacement);
						adresse = intToHex(tmpi);
	...
	private void instr_org(String adresse) {
		int id = hexToDecimal(adresse.substring(1)) - 5120;
		if (id >= 0 && id <= 1024) {
			idrom = (int) (id);
		} else {
			afficherErreur("Votre programme comporte des erreurs.");
	...
	private void instr_or(char registre, String[] mots) {
		if (mots[1].charAt(0) == '#' && mots[1].charAt(1) == '$') { // Mode d'adressage immediat
			...
			case 'A':
				...
				flagNeg(intToHex(newa).substring(sizea - 2), 8);
				flagZero(intToHex(newa).substring(sizea - 2));
			...
			case 'B':
				...
				ArchitecteInterne.setB(intToHex(newb).substring(sizeb - 2));
				flagNeg(intToHex(newb).substring(sizeb - 2), 8);
				flagZero(intToHex(newb).substring(sizeb - 2));
			...
			if (size == 5) // Mode d'adressage etendu
			{
				...
				case 'B':
					...
					idrom++;
					break;
				case 'G': // La command ORG $B000 pour declare la 1er adresse pour le programme dans la
							// ROM
					instr_org(adresse);
					break;
				default:
	...
	private void instr_asr(char registre) {
		...
		case 'A':
			...
			flagNeg(intToHex(valintA).substring(2), 8);
			flagZero(intToHex(valintA).substring(2));
		...
		case 'B':
			...
			flagNeg(intToHex(valintB).substring(2), 8);
			flagZero(intToHex(valintB).substring(2));
	...
	private void instr_inc(char registre) {
		...
		case 'B':
			...
			ArchitecteInterne.setA(valBB.substring(2));
			if (valintB > 255) {
				ArchitecteInterne.setC("1");
				ArchitecteInterne.setV("1");
			}
